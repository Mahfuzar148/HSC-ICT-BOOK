...
\item একটি basic HTML file লিখে \texttt{.html} extension দিয়ে save করতে।
...
HTML tag, element ও attribute জানা থাকলে একটি সম্পূর্ণ web page-এর গঠন বোঝা সহজ হয়। HTML file-এর code এলোমেলোভাবে লেখা যায় না; browser যেন document-এর শুরু, page title, metadata এবং visible content ঠিকভাবে বুঝতে পারে, সে জন্য একটি নির্দিষ্ট structure অনুসরণ করা হয়।

HSC ICT-তে basic HTML program লেখার সময় সাধারণত \texttt{<html>}, \texttt{<head>}, \texttt{<title>} এবং \texttt{<body>} অংশ ব্যবহার করে web page তৈরি দেখানো হয়। আধুনিক HTML-এ এর শুরুতে \texttt{<!DOCTYPE html>} এবং বাংলা text ঠিকভাবে দেখানোর জন্য \texttt{<meta charset="UTF-8">} ব্যবহার করা ভালো।


\begin{definitionbox}{HTML Document Structure}
HTML Document Structure হলো একটি HTML web page-এর মৌলিক গঠন, যেখানে কোন tag কোথায় থাকবে এবং browser কোন অংশ কীভাবে বুঝবে তা নির্ধারিত থাকে।
\end{definitionbox}
...
\begin{itemize}
\item \texttt{<html>} tag-এ \texttt{lang} attribute দিলে page-এর ভাষা বোঝা সহজ হয়।
...
\item \texttt{<head>} অংশে title, metadata, CSS link ও script link রাখা হয়।
\end{itemize}
\end{keypointbox}
...
\texttt{<!DOCTYPE html>} হলো document type declaration, যা browser-কে জানায় যে document-টি modern HTML বা HTML5 standard অনুসরণ করে লেখা হয়েছে।
\end{definitionbox}

\begin{itemize}
\item এটি HTML tag নয়, declaration।
\item সাধারণত HTML document-এর একদম শুরুতে লেখা হয়।
\item Browser-কে standard mode-এ page render করতে সাহায্য করে।
\item HSC basic code-এ অনেক সময় না দেখালেও modern HTML code-এ এটি লেখা ভালো।
\end{itemize}

\subsubsection{\texttt{<html>} tag}

\begin{definitionbox}{Root element}
\texttt{<html>} tag হলো HTML document-এর root element; এর ভিতরে সম্পূর্ণ web page-এর code লেখা হয়।
\end{definitionbox}
...
\texttt{<head>} tag হলো HTML document-এর এমন অংশ যেখানে page সম্পর্কিত তথ্য, title, style, script, link এবং meta information রাখা হয়।
\end{definitionbox}
...
\begin{itemize}
\item এই অংশের content সাধারণত browser window-এর main page area-তে সরাসরি দেখা যায় না।
\item Page title, character encoding, CSS link, script link ইত্যাদি এখানে লেখা যায়।
...
\item এটি \texttt{<head>} tag-এর ভিতরে লেখা হয়।
...
\begin{itemize}
\item এটি \texttt{<head>} tag-এর ভিতরে লেখা হয়।
\item এটি page body-এর ভিতরে দেখা যায় না।
...
\hline
\textbf{Tag} & \textbf{কোথায় দেখা যায়} \\
\hline
...
\texttt{<body>} tag হলো HTML document-এর প্রধান অংশ, যেখানে web page-এর দৃশ্যমান content লেখা হয়।
\end{definitionbox}
...
\begin{itemize}
\item Web page-এ যে text, image, link, table, list বা form দেখা যায়, তা \texttt{<body>} অংশে লেখা হয়।
\item \texttt{<body>} tag-এর বাইরে visible heading বা paragraph লেখা উচিত নয়।
\item HSC practical code-এ body অংশ সবচেয়ে বেশি ব্যবহৃত হয়।
\end{itemize}
...
\hline
\textbf{বিষয়} & \textbf{\texttt{<head>}} & \textbf{\texttt{<body>}} \\
\hline
কাজ & Page সম্পর্কিত তথ্য রাখে & Page-এর visible content রাখে \\
\hline
Browser-এ সরাসরি দেখা যায়? & সাধারণত না & হ্যাঁ \\
\hline
...
\hline
HSC focus & setting, style, script, title & browser-এ যা দেখা যায় \\
\hline
\end{tabular}
\end{center}
...
flowchart TD
    A[html] --> B[head]
    A --> C[body]
    B --> D[title, meta, style]
    C --> E[heading, paragraph]
    C --> F[image, link, list, table, form]
\end{verbatim}
\end{examplebox}

\begin{keypointbox}{Tree থেকে code বোঝার নিয়ম}
Tree diagram-এ parent tag-এর ভিতরে child tag থাকে। যেমন \texttt{<html>} tag-এর ভিতরে \texttt{<head>} ও \texttt{<body>} থাকে। আবার \texttt{<body>} tag-এর ভিতরে visible content যেমন heading, paragraph, image, link, list ও table লেখা হয়।
\end{keypointbox}
...
\hline
\texttt{<title>} & browser tab title দেখায় \\
\hline
\texttt{<body>} & visible content রাখে \\
\hline
\texttt{<h1>} & বড় heading দেখায় \\
\hline
\texttt{<p>} & paragraph দেখায় \\
\hline
\end{tabular}
\end{center}

\subsection{HTML File Save করার নিয়ম}

HTML code লেখার পর file save করতে হবে \texttt{.html} বা \texttt{.htm} extension দিয়ে।
...
\item File browser দিয়ে open করো।
...
\item closing tag না দেওয়া।
\item file \texttt{.txt} হিসেবে save করা।
\item বাংলা text ঠিকভাবে দেখানোর জন্য \texttt{UTF-8} না দেওয়া।
...
HTML document একটি নির্দিষ্ট কাঠামো অনুসরণ করে লেখা হয়। আধুনিক HTML document-এর শুরুতে \texttt{<!DOCTYPE html>} লেখা হয়। এরপর \texttt{<html>} tag-এর মধ্যে সম্পূর্ণ document থাকে। \texttt{<head>} অংশে web page সম্পর্কিত তথ্য, title, metadata, style ও script রাখা হয়। \texttt{<title>} tag browser tab-এ page title প্রদর্শন করে। \texttt{<body>} অংশে web page-এর দৃশ্যমান content যেমন heading, paragraph, image, hyperlink, list, table ও form লেখা হয়।
...
\item HTML document-এর visible content কোন tag-এর ভিতরে লেখা হয়?\\
\textbf{উত্তর:} \texttt{<body>} tag-এর ভিতরে।

\item Web page-এর title browser tab-এ দেখাতে কোন tag ব্যবহৃত হয়?\\
\textbf{উত্তর:} \texttt{<title>} tag।

\item \texttt{<head>} tag-এর কাজ কী?\\
\textbf{উত্তর:} Page সম্পর্কিত তথ্য, title, metadata, style ও script রাখার জন্য \texttt{<head>} tag ব্যবহৃত হয়।

\item \texttt{<!DOCTYPE html>} কেন ব্যবহার করা হয়?\\
\textbf{উত্তর:} Browser-কে জানাতে যে document-টি modern HTML বা HTML5 standard অনুসরণ করে লেখা হয়েছে।
...
\item Browser tab-এ page title দেখাতে কোন tag ব্যবহৃত হয়?\\
...
\item Modern HTML document-এর শুরুতে কোন declaration লেখা হয়?\\
