...
ওয়েব ডিজাইন বুঝতে হলে তিনটি বিষয় ভালোভাবে জানা দরকার - World Wide Web বা WWW, Web Browser এবং Search Engine। কারণ HTML দিয়ে ওয়েবপেজ তৈরি করা হলেও user সেই ওয়েবপেজ দেখে web browser-এর মাধ্যমে। আবার নির্দিষ্ট website বা প্রয়োজনীয় তথ্য খুঁজে পাওয়ার জন্য search engine ব্যবহার করা হয়।

সংক্ষেপে বলা যায়, WWW হলো web resource-এর বিশাল ব্যবস্থা, browser হলো সেই resource দেখার software এবং search engine হলো WWW থেকে প্রয়োজনীয় তথ্য খুঁজে বের করার tool।

\begin{center}
\begin{tikzpicture}[
  browser/.style={draw, rounded corners, fill=gray!4, minimum width=10.8cm, minimum height=5.7cm},
...
\node[bar, minimum width=7.6cm] (urlbar) at ([xshift=-0.75cm,yshift=2.05cm]win.center) {\texttt{https://www.abccollege.edu.bd}};
\node[bar, minimum width=1.45cm] (reload) at ([xshift=4.55cm,yshift=2.05cm]win.center) {Go};
\node[content, minimum width=2.2cm, minimum height=0.52cm] (logo) at ([xshift=-3.15cm,yshift=1.25cm]win.center) {ABC College};
\node[content, minimum width=5.1cm, minimum height=0.52cm] (nav) at ([xshift=1.3cm,yshift=1.25cm]win.center) {Home \quad Notice \quad Result \quad Contact};
\node[content, minimum width=8.7cm, minimum height=1.05cm] (hero) at ([yshift=0.3cm]win.center) {Homepage content\\welcome text, image/banner, important notice};
\node[content, minimum width=4.1cm, minimum height=0.7cm] (link) at ([xshift=-2.35cm,yshift=-1.0cm]win.center) {Hyperlink: Result};
\node[content, minimum width=4.1cm, minimum height=0.7cm] (page) at ([xshift=2.35cm,yshift=-1.0cm]win.center) {Webpage area};
\node[bar, minimum width=6.1cm] (search) at ([yshift=-2.05cm]win.center) {Search box: \texttt{HSC ICT result}};

\node[label] (lbrowser) at ([xshift=-5.9cm,yshift=2.55cm]win.center) {Browser};
\node[label] (lurl) at ([xshift=-5.55cm,yshift=1.55cm]win.center) {URL bar};
\node[label] (lwebsite) at ([xshift=5.85cm,yshift=1.25cm]win.center) {Website name};
\node[label] (lnav) at ([xshift=5.75cm,yshift=0.45cm]win.center) {Navigation};
\node[label] (lhome) at ([xshift=-5.55cm,yshift=0.15cm]win.center) {Homepage};
\node[label] (lsearch) at ([xshift=-5.55cm,yshift=-2.35cm]win.center) {Search bar};
\node[label] (llink) at ([xshift=-5.65cm,yshift=-1.1cm]win.center) {Link};

\draw[arrow] (lbrowser) -- (win.north west);
\draw[arrow] (lurl) -- (urlbar.west);
\draw[arrow] (lwebsite) -- (logo.east);
\draw[arrow] (lnav) -- (nav.east);
\draw[arrow] (lhome) -- (hero.west);
\draw[arrow] (lsearch) -- (search.west);
\draw[arrow] (llink) -- (link.west);
\end{tikzpicture}\\[3pt]
...
\draw[arrow] (browser) -- node[left, font=\scriptsize]{opens} (site);
\draw[arrow] (search) -- node[right, font=\scriptsize]{finds} (site);
\draw[arrow] (site) -- node[left, font=\scriptsize]{part of} (www);
\draw[arrow] (browser) -- node[above, font=\scriptsize]{uses URL} (www);
\draw[arrow] (search) -- node[above, font=\scriptsize]{indexes} (www);
\end{tikzpicture}\\[3pt]
...
World Wide Web-কে সংক্ষেপে Web বলা হয়। Web-এর মাধ্যমে user hyperlink ব্যবহার করে এক webpage থেকে অন্য webpage-এ যেতে পারে। Website, webpage, hyperlink, URL, browser এবং web server - এগুলো WWW ব্যবস্থার গুরুত্বপূর্ণ অংশ।
...
WWW-এর ধারণা দেন ব্রিটিশ computer scientist Tim Berners-Lee। তিনি ১৯৮৯ সালে CERN-এ কাজ করার সময় World Wide Web-এর ধারণা উপস্থাপন করেন। এর মূল উদ্দেশ্য ছিল গবেষক ও বিজ্ঞানীদের মধ্যে সহজে তথ্য বিনিময়ের ব্যবস্থা তৈরি করা।
...
\hline
CERN & 1989--1991 & WWW-এর জন্মস্থান; গবেষকদের তথ্য বিনিময়ের প্রয়োজন থেকে Web project শুরু হয় \\
\hline
W3C & 1994 & Web standard উন্নয়ন, HTML/CSS/accessibility guideline রক্ষণাবেক্ষণে কাজ করে \\
\hline
...
\hline
1990 & প্রথম web server ও browser/editor তৈরির কাজ শুরু হয় \\
\hline
1991 & Web publicভাবে পরিচিত হতে শুরু করে; প্রথমদিকের webpage HTML দিয়ে তৈরি হয় \\
\hline
1994 & W3C প্রতিষ্ঠিত হয় web standard উন্নয়নের জন্য \\
\hline
...
WWW client-server পদ্ধতিতে কাজ করে। User browser-এ কোনো website address বা URL লিখলে browser web server-এ request পাঠায়। Server সেই request অনুযায়ী HTML, CSS, image, JavaScript ইত্যাদি file browser-এ পাঠায়। এরপর browser সেগুলো process করে user-এর সামনে webpage আকারে দেখায়।
...
\item browser দিয়ে WWW access করা যায়।
\item URL ব্যবহার করে web resource খুঁজে পাওয়া যায়।
\item HTTP/HTTPS protocol ব্যবহার করে তথ্য আদান-প্রদান হয়।
\item text, image, audio, video, animation ও document প্রদর্শন করা যায়।
\item এটি বিশ্বব্যাপী তথ্য প্রকাশ ও ব্যবহারের সহজ মাধ্যম।
\end{itemize}

\subsection{WWW ও Internet-এর পার্থক্য}

অনেক শিক্ষার্থী WWW ও Internet-কে একই মনে করে। কিন্তু এরা একই জিনিস নয়। Internet হলো বিশ্বব্যাপী computer network, আর WWW হলো সেই internet ব্যবহার করে webpage ও web resource access করার ব্যবস্থা।

\begin{center}
\begin{tabular}{|p{0.20\textwidth}|p{0.32\textwidth}|p{0.32\textwidth}|}
\hline
\textbf{বিষয়} & \textbf{Internet} & \textbf{WWW} \\
\hline
...
Internet হলো রাস্তা বা যোগাযোগের অবকাঠামো, আর WWW হলো সেই রাস্তা ব্যবহার করে চলা web service। তাই Internet না থাকলে WWW ব্যবহার করা যায় না, কিন্তু Internet শুধু WWW নয়; email, file transfer, online game, video call ইত্যাদিও Internet ব্যবহার করে।
\end{examplebox}
...
\item bookmark, history ও save সুবিধা দেওয়া।
\item webpage print করা।
\item form submit ও login page ব্যবহার করতে সাহায্য করা।
\end{itemize}
\end{keypointbox}

\subsubsection{Browser কীভাবে webpage দেখায়?}

একটি webpage browser-এ দেখানোর সাধারণ ধাপ হলো:

\begin{enumerate}
\item User browser-এ URL লেখে।
\item Browser DNS-এর সাহায্যে domain-এর IP address খুঁজে পায়।
\item Browser web server-এ request পাঠায়।
\item Server HTML, CSS, image ও অন্যান্য file পাঠায়।
\item Browser HTML code পড়ে webpage তৈরি করে।
...
\begin{tabular}{|p{0.20\textwidth}|p{0.32\textwidth}|p{0.32\textwidth}|}
\hline
\textbf{বিষয়} & \textbf{Web Browser} & \textbf{Search Engine} \\
\hline
অর্থ & webpage দেখার application software & তথ্য খোঁজার online tool/service \\
\hline
কাজ & website খুলে দেখায় & keyword অনুযায়ী website বা তথ্য খুঁজে দেয় \\
\hline
ব্যবহার & URL লিখে website visit করা & keyword লিখে result পাওয়া \\
\hline
উদাহরণ & Chrome, Firefox, Edge, Safari & Google, Bing, Yahoo, DuckDuckGo \\
\hline
সম্পর্ক & search engine access করতে browser লাগে & browser-এর মাধ্যমে search engine ব্যবহার করা হয় \\
\hline
\end{tabular}
\end{center}
...
ইন্টারনেট থেকে প্রয়োজনীয় তথ্য, webpage, image, video বা document খুঁজে পাওয়ার জন্য ব্যবহৃত web-based tool বা software system-কে search engine বলে।
\end{definitionbox}

Search engine user-এর দেওয়া keyword অনুযায়ী internet থেকে relevant website বা webpage খুঁজে result হিসেবে দেখায়।
...
\end{tikzpicture}\\[3pt]
\textit{চিত্র: Search engine keyword নিয়ে ranked result দেখায়}
\end{center}
...
\item \textbf{Indexing:} খুঁজে পাওয়া webpage-এর তথ্য search engine নিজের database-এ সাজিয়ে রাখে।
\item \textbf{Ranking/Searching:} user keyword লিখলে search engine index থেকে সবচেয়ে relevant page result হিসেবে দেখায়।
\end{enumerate}
...
\begin{itemize}
\item দ্রুত তথ্য পাওয়া যায়।
\item keyword দিয়ে সহজে খোঁজা যায়।
\item বিভিন্ন website-এর তথ্য একসাথে দেখা যায়।
\item image, video, news ও map আলাদাভাবে খোঁজা যায়।
\item শিক্ষার্থী ও গবেষকের জন্য সহায়ক।
\item নির্দিষ্ট website address না জানলেও তথ্য পাওয়া যায়।
\end{itemize}

\subsubsection{Search Engine ব্যবহারে সতর্কতা}

Search engine অনেক result দেখায়, কিন্তু সব তথ্য সঠিক নাও হতে পারে। তাই reliable source যাচাই করা দরকার।
...
\item একাধিক source মিলিয়ে দেখা ভালো।
\item advertisement ও actual result আলাদা বুঝতে হবে।
\item copy-paste না করে নিজের ভাষায় লিখতে হবে।
\end{itemize}
\end{keypointbox}

\subsection{WWW, Browser ও Search Engine-এর সম্পর্ক}

এই তিনটি বিষয় পরস্পরের সাথে সম্পর্কিত।
...
\item Browser দিয়ে webpage দেখা হয়।
\item Search engine দিয়ে internet থেকে তথ্য খোঁজা হয়।
\item Chrome browser, কিন্তু Google Search search engine।
\item Website visit করতে HTTP/HTTPS protocol ব্যবহৃত হয়।
\end{itemize}
\end{keypointbox}
...
\begin{boardbox}{উত্তর}
Internet থেকে প্রয়োজনীয় তথ্য, webpage, image, video বা document খুঁজে পাওয়ার জন্য ব্যবহৃত web-based tool বা software system-কে search engine বলে। যেমন: Google, Bing, Yahoo ইত্যাদি।
\end{boardbox}
...
\item Website দেখার জন্য সাধারণত কোন software ব্যবহৃত হয়?\\
...
\item খ. Search engine কেন ব্যবহার করা হয়?
...
\textbf{খ.} Search engine internet থেকে প্রয়োজনীয় তথ্য বা website খুঁজে পেতে সাহায্য করে।\\
\textbf{গ.} Google search engine হিসেবে college website খুঁজে দিয়েছে; browser website খুলে homepage প্রদর্শন করেছে।\\
\textbf{ঘ.} WWW হলো web resource-এর system, browser সেই resource দেখায়, আর search engine WWW থেকে প্রয়োজনীয় resource খুঁজে দেয়।
\end{boardbox}
...
\item URL না জানলে search engine, আর URL জানা থাকলে browser address bar ব্যবহার করা যায় - এটি না বোঝা।
\end{itemize}
...
\item Web browser দিয়ে webpage দেখা হয়।
\item Search engine দিয়ে Internet থেকে তথ্য খোঁজা হয়।
\item Chrome browser, কিন্তু Google Search search engine।
\item Browser URL ব্যবহার করে server-এ request পাঠায় এবং webpage দেখায়।
\end{itemize}
\end{recapbox}
