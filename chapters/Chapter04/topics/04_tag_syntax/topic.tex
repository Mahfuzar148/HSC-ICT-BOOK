...
HTML document-এ content-এর ধরন বা কাজ বোঝাতে angle bracket \texttt{< >}-এর মধ্যে যে keyword লেখা হয় তাকে tag বলে। যেমন \texttt{<p>} paragraph শুরু করে এবং \texttt{</p>} paragraph শেষ করে।
\end{definitionbox}

\begin{definitionbox}{Content}
Start tag ও end tag-এর মাঝখানে যে text, image, link বা অন্য element থাকে তাকে content বলা হয়। যেমন \texttt{<h1>Welcome to ICT</h1>}-এ \texttt{Welcome to ICT} content।
\end{definitionbox}
...
Tag-এর অতিরিক্ত তথ্য বা property নির্দেশ করতে attribute ব্যবহৃত হয়। Attribute সাধারণত opening tag-এর মধ্যে লেখা হয়। যেমন \texttt{<a href="page.html">Link</a>}-এ \texttt{href} attribute।
\end{definitionbox}
...
\begin{tikzpicture}[
  ...
  box/.style={draw, rounded corners, align=center, inner sep=5pt, text width=34mm, fill=gray!5}
]
\node[box] (element) {\textbf{HTML element}\\\texttt{<p align="center">Bangladesh</p>}};
\node[box, above left=of element] (starttag) {\textbf{Start tag}\\\texttt{<p align="center">}};
\node[box, above right=of element] (endtag) {\textbf{End tag}\\\texttt{</p>}};
\node[box, below=of element] (attr) {\textbf{Attribute}\\\texttt{align="center"}};
...
যে HTML tag-এর সাধারণত কোনো closing tag থাকে না এবং যা content ঘিরে রাখে না, তাকে empty tag বলে। আধুনিক HTML-এ এগুলোকেই প্রায়শ void element বলা হয়। যেমন \texttt{<br>}, \texttt{<hr>}, \texttt{<img>}।
\end{definitionbox}

\begin{keypointbox}{Source PDF মিলিয়ে important empty tag}
HSC MCQ-তে empty tag শনাক্ত করতে বলা হয়। মনে রাখবে:
\begin{itemize}
...
HTML code লেখার নিয়মকেই HTML syntax বলে। Syntax ঠিক না হলে browser অনেক সময় code-কে সঠিকভাবে বুঝতে পারে না।

\begin{definitionbox}{Syntax}
HTML document-এ tag, element, attribute ও content সঠিকভাবে লেখার নিয়মকে syntax বলে।
\end{definitionbox}
...
\texttt{<empty-tag>}\\[2pt]
Tag সাধারণত angle bracket-এর মধ্যে থাকে, attribute opening tag-এর মধ্যে থাকে, আর closing tag-এ slash \texttt{/} ব্যবহার করা হয়।
}}
\end{center}

\begin{keypointbox}{HTML syntax-এর প্রধান নিয়ম}
\begin{itemize}
...
একটি HTML element-এর content হিসেবে আরেকটি HTML element ব্যবহার করা হলে তাকে nested HTML element বলা হয়।
\end{definitionbox}

Nested element লিখতে হলে tag close করার order ঠিক রাখতে হয়। যে tag আগে শুরু হয়েছে, সাধারণত তার ভিতরের tag আগে close হবে, তারপর বাইরের tag close হবে।
...
এখানে \texttt{<i>} tag \texttt{<b>} tag-এর ভিতরে আছে। তাই আগে \texttt{</i>} এবং পরে \texttt{</b>} লেখা হয়েছে। এটি correct nesting।
\end{examplebox}

\begin{mistakebox}{ভুল nesting}
\texttt{<b><i>ICT</b></i>} ভুল, কারণ এখানে \texttt{<i>} tag আগে close হওয়ার কথা ছিল, কিন্তু \texttt{</b>} আগে লেখা হয়েছে। তাই nesting order ভুল।
\end{mistakebox}
...
\end{verbatim}
এখানে \texttt{align} হলো attribute এবং \texttt{center} হলো attribute value। বইয়ের উদাহরণে এই ধরনের লেখা board question-এ খুবই দেখা যায়।
...
\end{verbatim}
এখানে \texttt{href} destination page নির্দেশ করে, আর \texttt{target} link কোথায় খুলবে তা বোঝায়।
...
\end{verbatim}
\texttt{img} একটি empty tag; তাই এর closing tag নেই। \texttt{src} image-এর location দেখায় এবং \texttt{alt} বিকল্প text দেয়।
...
\item Attribute value quote ছাড়া লেখা।
\item Closing tag-এ \texttt{/} না দেওয়া।
\item Empty tag-এর closing tag লিখতে যাওয়া।
\item Tag nesting উল্টে দেওয়া।
...
\end{verbatim}
\texttt{<!DOCTYPE html>} browser-কে document type জানায়; \texttt{<html>} পুরো document ধরে; \texttt{<head>} metadata/title রাখে; \texttt{<body>} দৃশ্যমান content রাখে।
...
\end{verbatim}
\texttt{<h1>} সবচেয়ে বড় heading, আর \texttt{<p>} paragraph তৈরি করে।
...
<hr>
\end{verbatim}
\texttt{<br>} নতুন line তৈরি করে; \texttt{<hr>} বিষয়কে আলাদা করতে একটি horizontal line দেয়।
...
<!-- This is a comment -->
\end{verbatim}
Comment ব্রাউজারে দেখা যায় না; এটি code বুঝতে সাহায্য করে।
\end{examplebox}

\begin{boardbox}{বোর্ড ফোকাস}
Tag, element, attribute ও syntax নিয়ে প্রশ্নে প্রথমে সংজ্ঞা, পরে একটি ছোট code example এবং শেষে প্রতিটি অংশের কাজ লিখলে উত্তর পরিষ্কার হয়। বিশেষ করে \texttt{<p align="center">ICT</p>} এবং \texttt{<img src="..." alt="...">} type example ভালোভাবে অনুশীলন করো।
\end{boardbox}
