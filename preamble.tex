...
\usepackage{fancyhdr}
\usepackage{setspace}
\setstretch{1.3} % Better line spacing for Bengali text
\ifPDFTeX
...
\else
  ...
  % Primary Bangla font (Kalpurush). Progitvide fallbacks for missing shapes
...
\usepackage{float}
\usepackage{xcolor}
% geometry already configured above (avoid option clash)
\usepackage{enumitem}
...
% Improve PDF outlines/bookmarks and ensure outlines regenerate correctly
% `bookmark` provides robust bookmark handling (works with hyperref).
\usepackage{bookmark}
...
\pagestyle{fancy}
\fancyhf{}
% Ensure header space is sufficient for fancyhdr to avoid warnings
...
% Fix for chapter pages having no headers
...
% Ensure headheight remains sufficient even after package/class adjustments
\AtBeginDocument{\setlength{\headheight}{18pt}}
